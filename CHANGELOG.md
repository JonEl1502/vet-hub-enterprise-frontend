...
- **What changed:** an **After-hours surcharge** rendered as a full grooming SERVICE DETAILS card —
  ...
  the real service on a grooming visit (user: *"no After-hours surcharge, it should just appear in
  billing"*).
  - Visit-level fee lines (after-hours / walk-in surcharge, house-call call-out and travel) now
    carry their own category, `VISIT_FEE_CATEGORY = 'Fees & Surcharges'`, instead of borrowing the
    encounter's. A synthetic category, like the existing `Grooming Discount` / `Procedure
    Adjustment` lines — no catalog row.
  - `isVisitFeeTask` in `components/clinic/shared/visitFees.ts` is now the single definition of
    "this is a fee, not clinical work"; `useVisitWizard` uses it instead of its own copy of the
    name regex.
- **Why:** the fee borrowed `Grooming` so the wizard would stop stapling a "Vet Visit — clinical"
  chip onto boarding visits (2026-08-02). That fixed one mis-read by causing another: the backend
  auto-creates a module record for every task in a module category.
- **Record impact:** 🟢 None — billing is untouched. The task, its price and its bill line are
  exactly as before; only the heading they group under changes.
- **Data dependency:** none. Category is a free string on `appointment_tasks`.
- ⚠️ The name test inside `isVisitFeeTask` exists **only** for visits created before the category
  did. It is safe because these names are generated by the app — do not extend it to
  clinic-entered catalog names (see the vaccine-name trap on the session board).
...
- **What changed:** (user)
  ...
  credit* cell and the *Credit available* stat card, on the client AND patient profiles
  (patient tops up the owner's account). New `CreditTopUpModal`: amount, method chips
  (**M-Pesa · Cash · Bank · Card**), optional reference, and a live *credit after* line.
  It posts `clientsAPI.recordAdvance`, so the copy says **record**, never charge — the
  front desk has already taken the money. The panel states plainly that card and M-Pesa
  capture are not wired yet.
  ② **The report's sign-off moved below everything.** *Attending Veterinarian · Date ·
  Generated by VetHub Core* sat inside `MedicalReport`, with the Diagnostic Record and
  every other module record printing UNDERNEATH it — so the vet appeared to have signed
  off on half the document. It is now an exported `ReportSignOff`, rendered once after all
  sections; `MedicalReport` takes `hideSignature`.
- **Record impact:** 🔵 Low — a top-up writes an advance transaction (the same call the
  Payments tab's "Record advance" already made).
- **Data dependency:** None.
- **Rollback:** revert the commit and rebuild.
- ⚠️ **Watch out:** the top-up does NOT initiate an STK push or a card charge. It records
  money already in hand. Wiring the gateways is separate work, and until then the method
  is a label on the record, not a payment route.
