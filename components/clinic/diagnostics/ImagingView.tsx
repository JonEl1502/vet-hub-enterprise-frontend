...
const ImagingView: React.FC<Props> = ({ onOpenAppointment }) => {
  ...
  const startNew = () => { setEditing({ petId: null, petName: '', source: 'INTERNAL' as DiagSource, externalSource: '', modality: 'XRAY' as ImagingModality, bodyPart: '', findings: '', studyDate: new Date().toISOString().slice(0, 10), images: [] }); setPetSearch(''); };

  const addImage = async (file?: File) => { if (!file) return; setUploading(true); try { const url = await downscale(file); setEditing((d: any) => ({ ...d, images: [...d.images, url] })); } catch { toast.error('Image failed'); } finally { setUploading(false); } };

  const save = async () => {
    if (!editing.petId) { toast.error('Select a patient'); return; }
    setSaving(true);
    try {
      const res = await imagingAPI.create({ petId: editing.petId, source: editing.source, externalSource: editing.externalSource || undefined, modality: editing.modality, bodyPart: editing.bodyPart || undefined, findings: editing.findings || undefined, studyDate: editing.studyDate || undefined, images: editing.images } as any);
      ...
      {editing ? (
        <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-5 shadow-sm space-y-4 max-w-2xl">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div><label className={labelCls}>Body part</label><input className={fieldCls} value={editing.bodyPart} onChange={e => setEditing({ ...editing, bodyPart: e.target.value })} placeholder="Thorax, L femur…" /></div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {editing.images.map((url: string, i: number) => <div key={i} className="relative"><img src={url} className="w-20 h-20 rounded-lg object-cover border border-slate-200 dark:border-zinc-800" /><button onClick={() => setEditing({ ...editing, images: editing.images.filter((_: any, j: number) => j !== i) })} className="absolute -top-1.5 -right-1.5 bg-rose-500 text-white rounded-full p-0.5"><X size={11} /></button></div>)}
              <label className="w-20 h-20 rounded-lg border-2 border-dashed border-slate-200 dark:border-zinc-700 flex items-center justify-center cursor-pointer hover:border-seafoam">{uploading ? <Loader2 size={18} className="animate-spin text-seafoam" /> : <ImagePlus size={18} className="text-slate-400" />}<input type="file" accept="image/*" className="hidden" onChange={e => addImage(e.target.files?.[0])} /></label>
            </div>
          </div>
          <div><label className={labelCls}>Findings</label><textarea rows={3} className={fieldCls} value={editing.findings} onChange={e => setEditing({ ...editing, findings: e.target.value })} placeholder="Radiographic interpretation…" /></div>
