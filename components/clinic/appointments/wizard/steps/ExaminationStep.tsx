        ...
        <Section icon={Eye} title="Systemic Examination">
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-2">
            {SYSTEMS.filter(sys => show(`sys.${sys.slug}`)).map(sys => {
              ...
              // was written. The slug drives visibility only.
              const s = systems[sys.label] || {};
              const abnormal = !!(s.findings && s.findings.trim());
              return (
                <div key={sys.slug} className={`border rounded-xl p-2.5 space-y-1.5 transition-all ${abnormal ? 'border-amber-300 dark:border-amber-800 bg-amber-50/40 dark:bg-amber-950/20' : 'border-slate-200 dark:border-zinc-800'}`}>
                  <div className="flex items-center justify-between gap-2">
                    <p className="text-[9px] font-black uppercase tracking-widest text-pine dark:text-zinc-100">{sys.label}</p>
                    <button type="button" onClick={() => setSystem(sys.label, { normal: !s.normal, findings: s.normal ? s.findings : '' })}
                  ...
                  <input className="field-input !h-8 text-xs" placeholder="Enter findings if abnormal" value={s.findings ?? ''}
                    onChange={e => setSystem(sys.label, { findings: e.target.value, normal: e.target.value ? false : s.normal })} />
                </div>
              );
            })}
          </div>
