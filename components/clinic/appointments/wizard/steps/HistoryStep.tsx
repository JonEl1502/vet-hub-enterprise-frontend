...
import { Section, L, Seg , showsField } from '../fields';
...
const HistoryStep: React.FC<StepProps> = ({ data, setData, visibleFields }) => {
    ...
    <div className="space-y-4">
      <Section icon={FileText} title="Presenting Complaint">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {show('chiefComplaint') && <L label="Chief complaint / reason for visit" required className="md:col-span-1">
          ...
          {show('duration') && <L label="Duration" required>
        ...
        {show('presentIllness') && <L label="History of present illness" required>
        ...
        {show('currentMedication') && <L label="Current medication">
      ...
      <Section icon={Utensils} title="Diet & Intake">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          {show('appetite') && <L label="Appetite"><Seg options={APPETITE} value={d.appetite} onChange={v => setData({ appetite: v })} /></L>}
          {show('waterIntake') && <L label="Water intake"><Seg options={WATER} value={d.waterIntake} onChange={v => setData({ waterIntake: v })} /></L>}
        </div>
      </Section>

      <Section icon={Droplets} title="Elimination">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {show('urination') && <L label="Urination"><Seg options={URINATION} value={d.urination} onChange={v => setData({ urination: v })} /></L>}
          {show('defecation') && <L label="Defecation"><Seg options={DEFECATION} value={d.defecation} onChange={v => setData({ defecation: v })} /></L>}
        </div>
      </Section>

      <Section icon={Globe2} title="Environment & Background">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {show('environment') && <L label="Environment">
          ...
          {show('indoorOutdoor') && <L label="Indoor / outdoor"><Seg options={['Indoor', 'Outdoor', 'Both']} value={d.indoorOutdoor} onChange={v => setData({ indoorOutdoor: v })} /></L>}
          {show('travelHistory') && <L label="Travel history">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {show('vaccinationStatus') && <L label="Vaccination status"><Seg options={['Up to date', 'Overdue', 'Unknown', 'None']} value={d.vaccinationStatus} onChange={v => setData({ vaccinationStatus: v })} /></L>}
          {show('parasiteControl') && <L label="Parasite control"><Seg options={['Up to date', 'Overdue', 'Unknown', 'None']} value={d.parasiteControl} onChange={v => setData({ parasiteControl: v })} /></L>}
          {show('previousIllness') && <L label="Previous illness / conditions">
