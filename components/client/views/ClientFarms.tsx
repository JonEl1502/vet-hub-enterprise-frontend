      ...
      {active && (
        <div className="cp-card relative p-4 sm:p-5">
          <div className="flex items-start justify-between gap-3 sm:pr-56">
            ...
            {/* ⚠️ The connect ACTION moved to Medical (2026-08-29). This stays
                as the shortcut into it — two places to do one thing is how the
                two start behaving differently. */}
            <button
              type="button"
              onClick={() => navigate('/client/farm/medical')}
              className="shrink-0 text-right group"
              title={active.clinic ? 'Manage this in Medical' : 'Connect a clinic in Medical'}
            >
              {active.clinic ? (
                <>
