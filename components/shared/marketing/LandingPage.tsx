...
/**
 * Scroll to an in-page section, smoothly.
 *
 * ⚠️ Offset is computed here rather than left to `scroll-mt-*`, because the nav
 * is `fixed` (~76px) and a raw `scrollTo` ignores scroll-margin.
 *
 * ⚠️ A NOTE FOR WHOEVER DEBUGS THIS NEXT: if you measure smooth scrolling from
 * an automated/background tab it will look completely broken — `scrollY` never
 * moves. That is not this code. A hidden tab gets no animation frames, so
 * `behavior: 'smooth'` (and any rAF tween) is a no-op there while a plain
 * `window.scrollTo(x, y)` still works, because it is synchronous. Check
 * `document.visibilityState` before concluding anything.
 */
